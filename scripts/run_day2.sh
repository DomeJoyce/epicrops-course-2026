...
# ─────────────────────────────────────────────────────────────────────────────
# Preflight — make sure the one-time setup + data download actually finished,
# so we fail fast with a clear message instead of half-running then dying on a
# missing sample sheet or missing Bismark index.
# ─────────────────────────────────────────────────────────────────────────────
n_fastq=$(ls "${RAW_DIR}"/*_1.fastq.gz 2>/dev/null | wc -l)
if [[ ! -f "${RAW_DIR}/samplesheet.tsv" || "${n_fastq}" -lt 6 ]]; then
    echo "!! Data not ready: found ${n_fastq}/6 samples and $( [[ -f "${RAW_DIR}/samplesheet.tsv" ]] && echo "a" || echo "NO" ) sample sheet in ${RAW_DIR}."
    echo "   Finish the download first (it writes samplesheet.tsv at the end):"
    echo "       bash \$DATA_DIR/download_data.sh"
    exit 1
fi
if [[ ! -d "${REF_DIR}/indices/bismark/Bisulfite_Genome" ]]; then
    echo "!! Bismark index missing in ${REF_DIR}/indices/bismark/."
    echo "   Build the reference first:"
    echo "       bash \$SCRIPTS_DIR/00_setup_reference.sh 4"
    exit 1
fi
